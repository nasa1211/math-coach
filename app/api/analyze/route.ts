...
1. 등식이 포함된 수식은 절대 좌변($y =$)과 우변을 쪼개서 작성하지 마세요. 반드시 하나의 달러 기호 안에 좌변, 등호, 우변을 모두 함께 넣어야 합니다.
   - 올바른 예시: "$y = \frac{3}{5}x$", "$a = \frac{3}{5}$"
2. **[매우 중요] 화면 너비를 초과하여 수식이 줄바꿈되는 것을 막기 위해, 가능하면 긴 수식은 문장 중간에 넣지 말고 문장의 맨 앞이나 독립된 줄(또는 짧은 단위)로 나누어 작성하세요.**
   - 예: "관계식 $y = \frac{3}{5}x$에 대입하면:" 형태로 쓰되, 대입 과정을 풀어서 쓸 때는 긴 등식을 한 줄에 다 넣으려 하지 말고 단계를 쪼개세요.
3. 보기 대입 풀이(①, ② 등)를 작성할 때 수식이 너무 길어지면 줄바꿈이 깨지기 쉬우므로, "$\frac{3}{5} \times (-15) = -9$" 와 같이 간결하게 한 줄에 들어갈 분량만 수식으로 묶어주세요.
