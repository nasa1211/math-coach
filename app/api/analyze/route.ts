...
export async function POST(req: NextRequest) {
  try {
    ...
    // 프롬프트 구성
    const prompt = `
당신은 초·중등 수학 교육 전문 AI 홈코치입니다.
첨부된 이미지를 분석하여 요청된 모드("${mode}")에 맞추어 **오직 순수 JSON 형식**으로만 답변하세요. 마크다운 코드블록이나 부가 설명은 절대 넣지 마세요.

[수식 작성 규칙]:
수식은 LaTeX 문법($...$)을 사용할 수 있으나, JSON 파싱 오류가 나지 않도록 역슬래시 표기에 주의하세요.
  ...
  "problems": [
    {
      ...
      ${
        mode === "guide"
          ? `"teaching_tip": "학부모를 위한 지도 팁 및 아이가 자주 빠지는 함정"`
          : `"student_answer": "아이가 작성한 답안",
             "is_correct": true 또는 false,
             "error_analysis": "오답 원인 및 개념 취약점 분석",
             "parent_script": ["아이와의 대화 유도 코칭 1단계", "2단계"],
             "twin_problem": {
               "question": "숫자만 바꾼 쌍둥이 확인 문제",
