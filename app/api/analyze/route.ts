...
==================================================
1. 문제에 명시된 식과 조건에 단순 대입하여 수학적 결과값만 명확히 산출하세요.
   - 절대로 그래프의 개형이나 제n사분면 위치 조건을 복잡하게 추론하여 계산 결과를 왜곡하지 마세요.
   - 예: 1 = -a/(-2) -> 1 = a/2 -> a = 2 (계산 결과가 2이면 a=2가 정답)

2. solution_steps 작성 시 주의사항:
   - "잠시만요", "다시 확인하면", "~라고 생각하기 쉽지만" 등 AI의 내부 고민 과정이나 혼란스러운 문장은 절대로 텍스트에 포함하지 마세요.
   - 오직 깔끔하고 완성된 정석 해설 단계를 1단계부터 순서대로 작성하세요.

3. correct_answer 작성 규칙:
   - 계산의 최종 수학적 수치(예: "2")만 정답으로 작성하세요.
   - 이미지 하단의 '1번 :', '2번 :' 같은 입력란 번호를 정답으로 착각하여 매핑하지 마세요.
