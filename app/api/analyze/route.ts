...
==================================================
1. 각 문제 분석 시, 반드시 먼저 문제의 수식/도형 조건에 맞춰 수학적으로 완전한 풀이(solution_steps)를 진행하세요.
2. 풀이 결과로 나온 최종 값/명제/성질을 보기 ①~⑤ 항목과 하나씩 엄격히 대조하세요.
3. 구한 결과와 보기가 완벽히 일치하는 번호를 최종 \`correct_answer\`로 결정하세요.
   - 단일 정답인 경우: "④"
   - 정답이 2개 이상(복수 정답)인 경우: "④, ⑤" (쉼표로 구분하여 모두 표기)
   - [오답 매핑 주의]: 풀이 결과가 109°이고 적용 성질이 (가)라면, (가)와 109°가 모두 표기된 보기 번호(예: ④)만 정확히 선택해야 합니다.
